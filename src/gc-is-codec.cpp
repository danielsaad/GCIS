...
int main(int argc, char *argv[]) {
    ...
    if (strcmp(mode, "-c") == 0) {
        ...
        auto start = timer::now();
        char *str = d.decode_saca_lcp(&SA, &LCP);
        auto stop = timer::now();
        ...
        if (!d.suffix_array_check(SA, (unsigned char*)str, (uint_t)n, sizeof(char), 0))
            ...
            std::cout << "isSorted!!\n";
        if (!d.lcp_array_check(SA, LCP, (unsigned char*)str, (uint_t)n, sizeof(char), 0))
            std::cout << "isNotLCP!!\n";
        else
            std::cout << "isLCP!!\n";
#endif
        ...
        //std::ofstream output(argv[3], std::ios::binary);
        //output.write((const char*) &n,sizeof(n));
        output1.write((const char*)SA,sizeof(sa_int32_t)*n);
        output1.close();
