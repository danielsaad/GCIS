#include "gcis_repair_hybrid.hpp"
#include "../external/malloc_count/malloc_count.h"
#include <cassert>
#include <cstring>
#include <fstream>
#include <iostream>

using namespace std::chrono;
using timer = std::chrono::high_resolution_clock;

void load_string_from_file(char *&str, char *filename, int_t &n) {
    std::ifstream f(filename, std::ios::binary);
    f.seekg(0, std::ios::end);
    n = f.tellg();
    f.seekg(0, std::ios::beg);
    str = new char[n];
    f.read(str, n);
    f.close();
};

int main(int argc, char *argv[]) {

#ifdef MEM_MONITOR
    mm.event("GC-IS Init");
#endif

    if (argc != 5) {
        std::cerr << "Usage: \n"
                  << argv[0]
                  << " -c <file_to_be_encoded> <output> <level limit>\n"
                  << argv[0]
                  << " -d <file_to_be_decoded> <output> <level_limit>\n";
        exit(EXIT_FAILURE);
    }

    int level_limit = atoi(argv[4]);
    // Dictionary type
    auto g = gcis_repair_hybrid();
    char *mode = argv[1];
    if (strcmp(mode, "-c") == 0) {
        int_t n;
        char *str;
        load_string_from_file(str, argv[2], n);
        std::ofstream output(argv[3], std::ios::binary);
        std::ofstream reduced_string("/tmp/reduced_string.gcis",
                                     std::ios::binary);

#ifdef MEM_MONITOR
        mm.event("GC-IS Compress");
#endif

        auto start = timer::now();
        g.encode(str, n, level_limit);
        auto stop = timer::now();

#ifdef MEM_MONITOR
        mm.event("GC-IS Save");
#endif

        cout << "input:\t" << n << " bytes" << endl;
        cout << "output:\t" << g.size_in_bytes() << " bytes" << endl;
        cout << "time: " << (double)duration_cast<seconds>(stop - start).count()
             << " seconds" << endl;

        g.serialize(output);
        g.serialize_reduced_string(reduced_string);
        output.close();
        reduced_string.close();
        delete[] str;
    } else if (strcmp(mode, "-d") == 0) {
        std::ifstream input(argv[2]);
        std::ifstream reduced_string("/tmp/reduced_string.gcis");
        std::ofstream output(argv[3], std::ios::binary);

#ifdef MEM_MONITOR
        mm.event("GC-IS Load");
#endif

        g.load(input);
        g.load_reduced_string(reduced_string);

#ifdef MEM_MONITOR
        mm.event("GC-IS Decompress");
#endif

        auto start = timer::now();
        char *str;
        int_t n;
        tie(str, n) = g.decode();
        auto stop = timer::now();

        cout << "input:\t" << g.size_in_bytes() << " bytes" << endl;
        cout << "output:\t" << n << " bytes" << endl;
        cout << "time: "
             << (double)duration_cast<milliseconds>(stop - start).count() /
                    1000.0
             << setprecision(2) << fixed << " seconds" << endl;

        output.write(str, n);
        input.close();
        output.close();
    }

#ifdef MEM_MONITOR
    mm.event("GC-IS Finish");
#endif

    return 0;
}
