...
def plot_extract(results_folder):
    
    graphs_folder = os.path.join(results_folder,'graphs')
    print('Creating graphs folder: ')
    os.makedirs(graphs_folder,exist_ok=True)
    with open(os.path.join(results_folder,'results_extract.csv')) as infile:
        next(infile)
        ...
        for line in reader:
            ...
            ax.legend(loc='best')
            # plt.show()
            ouf = os.path.join(graphs_folder,filename+'.pdf')
            fig.savefig(ouf,dpi=600,format='pdf')


'''
    argv[1] = results folder from extract
'''
if __name__ == "__main__":
    plot_extract(sys.argv[1])