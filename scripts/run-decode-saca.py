...
def run_decode_nong(input_folder_path,output_folder_path):
    ...
    for f in input_filename:
        ...
        print("SACA Nong: ",f)
        start_time = time.perf_counter() # Start time
        process = subprocess.run(['../bin/sais-nong', '-c', decompressed_file,output_file],
        stdout=subprocess.PIPE,stderr=subprocess.PIPE)
        end_time = time.perf_counter() # End time
        total_time += end_time - start_time
...
def run_decode_yuta(input_folder_path,output_folder_path):
    ...
    for f in input_filename:
        ...
        print("SACA Yuta: ",f)
        start_time = time.perf_counter() # Start time
        process = subprocess.run(['../bin/sais-yuta', '-c', decompressed_file,output_file],
        stdout=subprocess.PIPE,stderr=subprocess.PIPE)
        end_time = time.perf_counter() # End time
        total_time += end_time - start_time
...
def run_saca(input_folder_path,output_folder_path):
    ...
    decode_saca_nong_data = run_decode_nong(input_folder_path,output_folder_path)
    decode_saca_yuta_data = run_decode_yuta(input_folder_path,output_folder_path)
