...
#ifndef GC_IS_GCIS_S8B_NO_LCP_HPP
#define GC_IS_GCIS_S8B__NO_LCP_HPP

#include "gcis.hpp"
#include "gcis_s8b_no_lcp_codec.hpp"
#include "util.hpp"
template <>
class gcis_dictionary<gcis_s8b_no_lcp_codec> : public gcis_abstract<gcis_s8b_no_lcp_codec> {
  public:
    pair<char *, int_t> decode() override {
        ...
        for (int64_t i = g.size() - 1; i >= 0; i--) {
            ...
            gcis_s8b_no_lcp_pointers_codec_level gd = std::move(g[i].decompress());
  ...
  private:
    void gc_is(int_t *s, uint_t *SA, int_t n, int_t K, int cs, int level) {
        ...
        uint_t rule_index = 0;
        g.push_back(gcis_s8b_no_lcp_codec());
        ...
        for (i = 0; i < n1; i++) {
            ...
            if (diff) {
                ...
                g[level].rule_suffix_length.encode(cur_len );
                g[level].rule.resize(g[level].rule.size() + cur_len);


                for (j = 0; j < cur_len  && j + pos  < n; j++) {
...
class gcis_s8b_no_lcp_pointers : public gcis_dictionary<gcis_s8b_no_lcp_codec> {
  public:
    pair<char *, int_t> decode() override {
        sdsl::int_vector<> r_string = reduced_string;
        char *str = 0;
...
#endif // GC_IS_GCIS_S8B_NO_LCP_HPP
