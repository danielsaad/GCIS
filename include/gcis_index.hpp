...
int gcis_index<t_mapfbv, t_maptbv, t_mapwt, t_gridbv,
               t_gridwt>::cmp_suffix_rule(const gcis_index::rule_type &X,
                                          const std::string &str, len_type j) {
...
//    void gcis_index<_gcis>::find_secondary_occ(const gcis_index::_node & v,
...
//            t_wt::size_type n_occ =
//            sequence_tree.rank(sequence_tree.size(),v.first);
//
//            /**
...
//                    t_wt::size_type label =
//                    sequence_tree[loud_tree.node_id(parent_v)];
//                    /** Compute offset and insert the node to a future
//                    processing*/ auto off = S.front().second +
//                    _grammar->offset(label,ch_rank);
//                    S.emplace_back(label,(len_type)off);
//
//                    /** Compute total occ of the node */
//                    t_wt::size_type n_occ = sequence_tree.rank(
//                    sequence_tree.size(),label);
//
//                    for (int i = 1; i < n_occ ; ++i)
