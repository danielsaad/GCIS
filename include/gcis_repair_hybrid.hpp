...
class gcis_repair_hybrid : public gcis_s8b_pointers {
  public:
    int level_limit;

  public:
    void encode(char *s, int_t n, int level_limit) {
    ...
    void serialize(ostream &o) override {
    ...
    void serialize_reduced_string(ostream &o) {
        uint64_t size = reduced_string.size();
        o.write((char *)&size, sizeof(uint64_t));
        for (uint64_t i = 0; i < reduced_string.size(); i++) {
            int_t x = reduced_string[i];
            o.write((char *)&x, sizeof(int_t));
        }
    }

    void load(istream &i) override {
        uint64_t size;
    ...
    void load_reduced_string(istream &i) {
        uint64_t size;
        i.read((char *)&size, sizeof(uint64_t));
        reduced_string.resize(size);
        for (uint64_t j = 0; j < size; j++) {
            int_t x;
            i.read((char *)&x, sizeof(int_t));
            reduced_string[j] = x;
        }
    ...
    void gc_is(int_t *s, uint_t *SA, int_t n, int_t K, int cs,
        ...
        if (level == level_limit) {
            ...
            if (name + 1 < n1 && !premature_stop) {
...
#ifdef REPORT
                    gcis::util::print_report(
                        "Premature Stop employed at level ", level, "\n");
#endif
                ...
                sdsl::util::bit_compress(reduced_string);
#ifdef REPORT
                gcis::util::print_report("Reduced String Length = ",
                                         (int_t)reduced_string.size(), "\n");
                gcis::util::print_report(
                    "Reduced String Width (bits per symbol) = ",
                    (int_t)reduced_string.width(), "\n");
#endif
